...
#ifndef zprint_h_INCLUDED
#define zprint_h_INCLUDED
...
class ZPrint : public ZMode
{
  private:
    ...
    size_t writeChunk(const char * const s, int len);
  ...
  public:
    ...
    ZResult switchToPostScript(const char * const prefix);
    void setLastPrinterSpec(const char *spec);
    const char * const getLastPrinterSpec();
