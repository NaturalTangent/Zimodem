...
const char * const ZPrint::getLastPrinterSpec()
{
...
size_t ZPrint::writeChunk(const char * const s, int len)
{
  char buf[25];
  sprintf(buf,"%x\r\n",len);
  writeStr(buf);
...
ZResult ZPrint::switchToPostScript(const char * const prefix)
{
  if((lastPrinterSpec==0)
