...
WiFiServerNode::WiFiServerNode(uint16_t newport, int flagsBitmap)
{
  id=++WiFiNextClientId;
  port=newport;
...
bool WiFiServerNode::ReadWiFiServer(File &f, WiFiServerSpec &node)
{
  if(f.available()>0)
  {
    ...
    if(str.length()==0)
      return false;
    node.port = strtoul(str.c_str(), NULL, 10);
    str = "";
    c=f.read();
    while((c != '\n') && (f.available()>0))
    {
...
WiFiServerNode *WiFiServerNode::FindServer(uint16_t port)
{
  WiFiServerNode *s=servs;
  while(s != null)
  {
