...
const char *CONFIG_FILE_OLD = "/zconfig.txt";
const char *CONFIG_FILE     = "/zconfig_v2.txt";
...
ZResult ZCommand::doInfoCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  if(vval == 0)
  {
    showInitMessage();
  ...
  case 5:
  {
    ...
    {
      for(size_t i=0;i<strlen(maskOuts);i++)
...
ZResult ZCommand::doBaudCommand(uint32_t vval, uint8_t *vbuf, int vlen)
{
  if(vval<=0)
  {
...
ZResult ZCommand::doConnectCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
  if(vlen == 0)
  {
...
ZResult ZCommand::doWebStream(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *filename, bool cache)
{
  char *hostIp;
  char *req;
  int port;
  ...
  
  {
    ...
    if((binType != BTYPE_NORMAL_NOCHK) &&(machineQue.length()==0))
    {
      ...
      for(size_t i=0;i<flen;i++)
      {
        ...
        if(!cache)
        {
          for(uint32_t i1=0;i1<bufLen;i1++)
          {
            chk8+=buf[i1];
            if(chk8>255)
              chk8-=256;
...
ZResult ZCommand::doUpdateFirmware(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  ...
  uint8_t buf[255];
  uint32_t bufSize = 254;
  char firmwareName[100];
#ifdef USE_DEVUPDATER
  const char * const updaterHost = "192.168.1.10";
  int updaterPort = 8080;
#else
  const char * const updaterHost = "www.zimmers.net";
  int updaterPort = 80;
#endif
#ifdef ZIMODEM_ESP32
  const char * const updaterPrefix = "/otherprojs/guru";
#else
  const char * const updaterPrefix = "/otherprojs/c64net";
#endif
  ...
  serial.prints(".");
  serial.flush();
  size_t writeBytes = Update.writeStream(*c);
...
ZResult ZCommand::doWiFiCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
  bool doPETSCII = (strchr(dmodifiers,'p')!=null) || (strchr(dmodifiers,'P')!=null);
  if((vlen==0)||(vval>0))
  {
    uint32_t n = (uint32_t)WiFi.scanNetworks();
    if((vval > 0)&&(vval < n))
      n=vval;
...
ZResult ZCommand::doTransmitCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers, int *crc8)
{
  bool doPETSCII = (strchr(dmodifiers,'p')!=null) || (strchr(dmodifiers,'P')!=null);
  int crcChk = *crc8;
  *crc8=-1;
  ...
  {
    uint8_t buf[vval];
    uint32_t recvd = HWSerial.readBytes(buf,vval);
    if(logFileOpen)
    {
...
ZResult ZCommand::doDialStreamCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
  if(vlen == 0)
  {
...
ZResult ZCommand::doPhonebookCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
    ...
    while(phb != null)
    {
      ...
      ||(vval == phb->number))
      {
        ...
        || (modifierCompare(dmodifiers,phb->modifiers)==0))
        {
          ...
          serial.prints(nbuf);
          for(size_t i=0;i<10-strlen(nbuf);i++)
            serial.prints(" ");
          serial.prints(" ");
          serial.prints(phb->modifiers);
          for(uint32_t i=1;i<5-strlen(phb->modifiers);i++)
          {
            serial.prints(" ");
          }
...
ZResult ZCommand::doAnswerCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber, const char *dmodifiers)
{
  if(vval <= 0)
  {
      WiFiClientNode *c=conns;
...
ZResult ZCommand::doHangupCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  if(vlen == 0)
  {
    while(conns != null)
...
ZResult ZCommand::doLastPacket(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  if(!isNumber)
    return ZERROR;
...
ZResult ZCommand::doEOLNCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  if(isNumber)
  {
    if((vval>=0)&&(vval < 4))
...
ZResult ZCommand::doTimeZoneSetupCommand(uint32_t vval, uint8_t *vbuf, int vlen, bool isNumber)
{
      ...
      if(vlen>0)
      {
        ...
        if((vlen > 0)&&(isNumber))
        {
          ...
          for(uint8_t *v=vbuf;v<(vbuf+vlen);v++)
            if((*v>='0')&&(*v<='9'))
              finalNum += (char)*v;
          vval=strtoul(finalNum.c_str(), NULL, 10);
        }
      }
...
uint8_t *ZCommand::doStateMachine(uint8_t *buf, uint32_t *bufLen, char **machineState, String *machineQue, char *stateMachine)
{
  if((stateMachine != NULL) && ((stateMachine)[0] != 0) && (*machineState != NULL) && ((*machineState)[0] != 0))
  {
    String newBuf = "";
    for(uint32_t i=0;i<*bufLen;)
    {
...
uint8_t *ZCommand::doMaskOuts(uint8_t *buf, uint32_t *bufLen, char *maskOuts)
{
  if(maskOuts[0] != 0)
  {
    uint32_t oldLen=*bufLen;
    for(uint32_t i=0,o=0;i<oldLen;i++,o++)
    {
...
void ZCommand::reSendLastPacket(WiFiClientNode *conn)
{
  ...
  {
    uint32_t bufLen = conn->lastPacketLen;
