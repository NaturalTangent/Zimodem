    ...
    if(ch == '\n')
    {
      ...
      else
      {
        ...
        if(x>0)
        {
          ...
          {
            ln.remove(0,16);
            respLength = strtoul(ln.c_str(), NULL, 10);
          }
          else
          if(header == "location")
          {
...
bool doWebGetBytes(const char *hostIp, int port, const char *req, const bool doSSL, uint8_t *buf, uint32_t *bufSize)
{
  *bufSize = -1;
  uint32_t respLength=0;
  WiFiClient *c = doWebGetStream(hostIp, port, req, doSSL, &respLength);
  ...
  if(((!c->connected())&&(c->available()==0)) ||(respLength > *bufSize))
  {
    c->stop();
    delete c;
    return false;
